...
func LoadFishNormal(fType entities.FishList, level int) (*ebiten.Image, error) {
	...
	img, err := LoadImageAssetAsEbitenImage(normalImgPath)
	if err != nil {
		return nil, err
	}
	return img, nil
...
func LoadFishSprite(creatureType entities.FishList, creatureLvl int) (*sprite.AnimatedSprite, error) {
	...
	ani, sps, err := LoadAnimation(path)
	if err != nil {
...
func NewFish(environment *system.Environment, hub *tasks.EventHub, tankSize geometry.Rect, saveData entities.SavedFish) *entities.Creature {
