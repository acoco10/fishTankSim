...
func (cu *CursorUpdater) Draw(screen *ebiten.Image) {
	...
	screen.DrawImage(cu.cursorImages["Default"], dopts)*/
}
...
func loadCursorImages() map[string]*ebiten.Image {
	...
	imgMap["Default"] = ebiten.NewImageFromImage(img.SubImage(image.Rect(0, 0, 32, 32)))
	imgMap["NSResize"] = ebiten.NewImageFromImage(img.SubImage(image.Rect(0, 0, 32, 32)))
	imgMap["EWResize"] = ebiten.NewImageFromImage(img.SubImage(image.Rect(0, 0, 32, 32)))
