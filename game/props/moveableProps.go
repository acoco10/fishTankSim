...
func NewStructureProp(x float32, y float32, img *ebiten.Image, normal *ebiten.Image, hub *tasks.EventHub) *StructureProp {
	...
	if normal != nil {
		normalMapShader := shaders.LoadNormalMapShader()
...
func (p *StructureProp) Update() {
	...
	if p.state == Moveable {
		...
		p.X = max(float32(x), float32(p.boundaries.Min.X-p.Img.Bounds().Max.X))
		p.X = min(float32(x), float32(p.boundaries.Max.X-p.Img.Bounds().Max.X))
